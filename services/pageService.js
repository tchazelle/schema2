...
class PageService {
   ...
   * @param {Object} user - L'utilisateur (déjà enrichi par userEnrichMiddleware)
  ...
  static async getPageBySlug(slug, user) {
    ...
    if (!EntityService.canAccessEntity(user, 'Page', page)) {
      return null;
    }
  ...
  static async getPageSections(pageId, user) {
    ...
    for (const section of sections) {
      if (EntityService.canAccessEntity(user, 'Section', section)) {
        // Vérifier si l'utilisateur a accès à la table mentionnée dans la section
        if (section.tableName) {
          if (hasPermission(user, section.tableName, 'read')) {
  ...
  static async getAccessiblePages(user) {
    ...
    return EntityService.filterAccessibleEntities(user, 'Page', pages);
  }
